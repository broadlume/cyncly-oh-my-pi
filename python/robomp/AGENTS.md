...
- `entrypoint.sh` — creates slot users and `/data/{workspaces,logs}` + build caches; copies agent-config layer 1 then layer 2 into `/srv/agent-home`, per file. `worker._stage_agent_home()` repeats the same two-layer copy on every task.
